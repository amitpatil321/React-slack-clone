...
// TODO : Instead of getting rooms, currentRoom as 2 params,
// do it like({ rooms, room }) in hasRoom function
...
export function getJoinableUsers(rooms, currentRoom) {
  ...
  const generalRoom = filter(rooms, {
  ...
  // List all users except which are already member of this room
  {
    generalRoom[0].users.forEach((eachUser) => {
      if (!currentRoom.userIds.includes(eachUser.id.toString())) { allUsers.push(eachUser); }
    });
  }
  return allUsers;
}
...
export function peopleJoinedMessage(currentUser, channel, people, type) {
  ...
  const peopleNames = people.map(id => currentUser.userStore.users[id].name);
  ...
  if (currentUser !== undefined) {
    ...
    if (people.length) {
      if (people.length === 1) message = peopleNames[0];
        ...
        const last = peopleNames.pop();
        message = `${peopleNames.join(', ')} and ${last}`;
      }
      if (type === 'NEW') { return ` joined #${channel} along with ${message}`; }
...
export function getAllUsers(user) {
  // Find general room as We are storing all the users in 'general' room
  const generalRoom = filter(user.rooms, {
    id: process.env.REACT_APP_CHATKIT_GENERAL_ROOM,
  });
  if (generalRoom.length) { return orderBy(generalRoom[0].users, ['name'], ['asc']); }
  return false;
}
...
export function getDirectChatRoom(user, rooms, receiverId) {
  const roomUsers = [user.id, receiverId];
...
export function isPrivateChat(room) {
  if (room && room.customData !== undefined && room.customData.privateChat) { return true; }
  return false;
}
...
export function onlineStatus(user) {
  const status = user.presenceStore[user.id];
  return (
    <>
...
export function getLastMessageInRoom(room, messages) {
  if (room && messages && messages[room.id] !== undefined) {
    const msgArray = Object.values(messages[room.id]);
